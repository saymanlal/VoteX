    ...
    const handleVerify = async ()=>{
        try {
            ...
            if (response.ok) {
                ...
                setVerificationResult(`✅ Welcome ${data.admin.name} (Access ${data.admin.access})`);
            } else {
                setVerificationResult('❌ Invalid Admin ID');
            }
        } catch (err) {
            console.error(err);
            setVerificationResult('❌ Error verifying Admin ID');
        }
    };
...
function Hero() {
    ...
    // Function to start fade-out and hide overlay
    const startCloseOverlay = ()=>{
        if (isClosing) return; // prevent multiple triggers
        setIsClosing(true);
        // Wait for animation duration (0.5s) before fully hiding overlay
        setTimeout(()=>{
...
//# sourceMappingURL=%5Broot-of-the-server%5D__4ad91057._.js.map